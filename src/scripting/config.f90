...
contains
  ...
  subroutine create_fortshrc(home_dir)
    ...
    write(unit, '(a)') '# Line 1: user@host :: path, Line 2: prompt character'
    write(unit, '(a)') 'PS1=''%F{yellow}\u%f@%F{magenta}\h%f :: %F{cyan}\w%f\n> '''
    write(unit, '(a)') ''
    write(unit, '(a)') '# RPROMPT: Right-side prompt (like zsh) - shows date and time'
    write(unit, '(a)') 'RPROMPT=''%F{240}\D :: \A%f'''
    write(unit, '(a)') ''
    write(unit, '(a)') '# Alternative prompts (uncomment to use)'
    write(unit, '(a)') '# Single-line minimal: PS1=''\w> '''
    write(unit, '(a)') '# Single-line with :: : PS1=''\u@\h :: \w > '''
    write(unit, '(a)') '# Bash-style colors: PS1=''\[\e[1;32m\]\u@\h\[\e[0m\] :: \[\e[1;34m\]\w\[\e[0m\] > '''
    write(unit, '(a)') ''
